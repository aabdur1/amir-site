...
- **LivingField (site-wide data field).** `components/living-field.tsx`, mounted once in `layout.tsx`: a fixed full-viewport canvas of plot-like points (dots, axis ticks, ≤3 faint mono value labels) in both themes. Seeded PRNG; assembles from noise into a rising trend band on load; per-point scroll parallax (depth-scaled, wrapping) disperses the structure as you scroll; slow drift + cursor repel (fine pointers). 30fps cap, pauses when tab hidden, static assembled frame under reduced motion, hardcoded Latte/Mocha palettes (learn-artifact pattern). Route intensity via `routeProfile()`: full on `/`, medium on `/learn`, faint elsewhere (gallery/artifacts/404).
