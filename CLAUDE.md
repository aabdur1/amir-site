...
- **Client components marked explicitly.** Components using `"use client"`: nav, dark-mode-toggle, hero, animated-text, living-field, spine, count-up, interactive-headshot, certifications, experience, projects, featured-project, skills, education, footer, scroll-progress, page-transition, learn-teaser. Gallery components (masonry-grid, photo-card, sort-controls) are also client components. Learn components (learn-card, section-rail, gradient-descent, log-loss-cross-entropy, pca, regularization, clustering, shap, neural-networks, sql, python) are client components. learn-nav, section-header, section-divider, and spark-rule are server components.
...
- **Learn artifact components are code-split.** `app/learn/[slug]/page.tsx` uses `next/dynamic` to lazy-load each artifact component, preventing all 9 from bundling into a single chunk.
...
components/
  ...
  learn/
    ...
    dynamic-artifacts.tsx # Client wrapper for ssr: false dynamic imports (Log Loss, PCA, Clustering, SHAP, Neural Networks, SQL)
    ...
    python.tsx            # 09/ Python (4 sections: filtering, groupby, merging, windows — 11 checked pandas exercises on Pyodide, click-to-load from CDN)
lib/
...
scripts/
  ...
  generate-sql-seed.mjs   # Seed generator + teaching-shape assertions (writes lib/learn/sql-seed.ts AND lib/learn/python-data.ts from one PRNG stream)
public/
...
- `app/sitemap.ts` generates `/sitemap.xml` (homepage priority 1 monthly, gallery priority 0.8 weekly, learn index priority 0.8 monthly, 9 artifact pages priority 0.7 monthly)
